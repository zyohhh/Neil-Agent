...
- `/cockpit --live`：进入全屏实时驾驶舱；使用 `F3`（或 `Ctrl+T`）在执行 DAG 与五层上下文本地估算之间切换，`F2`（或 `Ctrl+O`）展开/恢复结果，`1`–`4` 在 DAG 模式筛选节点，`Ctrl+X` 取消请求、`Ctrl+Q` 退出。各区域会按终端宽高自适应；非交互终端或 Textual 启动失败时自动降级为基础快照。
