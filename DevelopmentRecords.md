...
- 当前版本：v0.1.0-dev
- 当前状态：模型请求重试和 `/doctor` 已完成，下一步支持项目指令和长会话压缩
...
下一阶段扩大为“项目指令和长会话连续性”，一次推进 5 项以加快开发：

1. 使用真实 DeepSeek API 验证限流/连接异常时的重试反馈，以及 `/doctor` 在正常和 Git 不可用场景下的输出。
2. 启动时安全读取工作区根目录的 `AGENTS.md`，设置文件大小和内容边界，并把项目指令追加到模型上下文但不写入会话快照。
3. 增加 `/instructions`，显示当前项目指令的来源、大小和是否生效，不展示系统提示词或其他隐藏配置。
4. 增加用户显式触发的 `/compact`，把较旧完整轮次总结为有标记的压缩上下文，保留最近轮次和工具链，并在失败时保持原历史不变。
5. 扩展 `evals/tasks.json` 和 CLI 集成测试，覆盖项目指令遵循、压缩前后任务连续性、工具审批与重试后的会话一致性。
