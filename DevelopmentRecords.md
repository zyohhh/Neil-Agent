...
## 2026-09-15：Web Output 显示修复

- 修复本地清空 Output 后持续隐藏新回答：记录已清空条目的边界，新流式内容继续显示，快照重同步保持边界，输出窗口淘汰旧条目后仍显示新内容。
- 新 `run_id` 重置上一轮 output、timeline 和 approval 投影；同一轮取消或完成保留当前输出。
- 修复回答分片逐条换行：显示时拼接连续正文，保留原文换行，使用 14 px 正文字体并自动折行；活动日志独立显示，清空边界与协议保持不变。
- 补齐 NA 头像文字的居中样式，重新生成打包静态资源与 SHA-256 清单。
- 验证：Web lint、14 项前端测试、TypeScript 检查与生产构建、5 项 Playwright 页面回归通过；两项新增回归在修复前复现问题，修复后通过。
- 增强回归覆盖中文分片、段落换行与穿插活动日志；修复前复现 7 个分片形成 7 行，修复后形成一个正文块。使用实际生产页面验证 1280 px 桌面和 390 px 手机上的短回答、长段落与长标识符，共 4 个可读性用例通过，并检查截图。
- 修复长回答被最近 200 个细碎分片覆盖：服务端和前端按最多 4000 字符合并连续正文，优先淘汰活动日志；单次较大回调拆分后完整保存，真正达到容量时保留开头并提示查看已保存历史。稳定条目 ID 与清空偏移支持合并后的快照重同步，Unicode 字符计数保持一致。
- 新增当前选中会话的认证只读问答分页，每页最多 32 个正文部分、每部分最多 4000 字符；会话 ID 与历史版本校验防止分页串会话。只投影问题与最终回答，排除工具、thinking、Provider 私有 payload 和内部压缩检查点。
- Workspace 显示已保存问答，点击旧 session 重载；长历史可继续加载，错误重试保留已加载内容，切换取消旧请求。更新开发协议、运维说明与项目状态，重新生成生产资源及 SHA-256 清单。
- 补齐清空后大量日志的回归：旧活动边界淘汰时，仍按每个已清空正文条目的 ID/偏移保持隐藏；快照刷新不使旧回答重新出现。新增断言先复现问题，修复后通过。
- 验证：离线门禁 850 passed、11 skipped、16 deselected；ruff、mypy、Web lint、20 项前端测试、TypeScript/生产构建及 5 项 Playwright 页面回归通过。生产页面在 1440 px 与 390 px 验证 4641 字符分页回答、5198 字符逐字流式回答和往返 session 切换，无页面溢出或运行错误，并检查截图。9 项分发测试通过，wheel 构建完成并验证新模块、历史 API 和全部前端资源哈希。
