...
下一阶段聚焦“上下文预算和本地会话管理”，仍不增加云同步：

1. 使用真实 DeepSeek API 验证退出进程后 `/sessions`、`/resume` 和恢复后继续调用工具。
2. 增加 `/context`，展示当前轮数、消息数和近似上下文占用，并为大型工具结果设置历史预算。
3. 增加需要明确确认的本地会话删除命令和存储占用展示，不自动删除历史会话。
