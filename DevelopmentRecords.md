...
- 当前版本：v0.1.0-dev
- 当前状态：根项目指令和显式长会话压缩已完成，下一步扩展分层指令与会话导航
...
| `instructions.py` | 安全读取工作区根目录的有界 `AGENTS.md` 项目指令 |
...
1. 使用真实 DeepSeek API 验证项目指令遵循、`/compact` 后继续调用工具与跨进程恢复，并观察一次自然发生的重试；不主动制造限流。
2. 支持从工作区根目录到目标文件目录的分层 `AGENTS.md`，定义累计大小上限、由外到内的优先顺序和符号链接边界。
3. 扩展 `/instructions`，按顺序显示所有生效来源及各自大小，并增加显式 `/reload-instructions`，加载失败时继续使用旧的有效快照。
4. 增加需要预览和明确批准的 `/init`，只在不存在根 `AGENTS.md` 时分析项目并生成一份简洁初稿，不覆盖已有文件。
5. 增加本地会话标题与搜索：自动生成不含模型调用的默认标题，并提供 `/rename-session`、`/sessions <query>`，保持版本迁移兼容。
6. 将 `evals/tasks.json` 接入可运行的离线评测入口，默认使用假模型和临时工作区，并提供显式开启的真实 DeepSeek 验收模式。
