...
- 当前版本：v0.1.0-dev
- 当前状态：主动质量验证和本地 Git 保存闭环已完成，下一步开发可见任务计划
...
下一阶段只聚焦“可见任务计划”，暂不增加远程 Git 或自动回滚：

1. 使用真实 DeepSeek API 验证修改后主动检查、结果汇总，以及 `git_stage` / `git_commit` 的批准和拒绝流程。
2. 为单次开发任务增加最多 5 步的可见计划，并记录 pending、in progress、completed 状态。
3. 增加 `/status` 命令，展示当前计划、最近一次质量检查和本地 Git 状态。
