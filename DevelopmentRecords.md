...
- Agent 在每次模型请求前立即报告“正在分析请求”或“正在根据工具结果继续处理”，减少请求期间无反馈的等待。
- 每个工具调用都会报告开始、完成、失败或跳过；完成和失败事件包含本次工具流程耗时。
- 工具活动使用面向用户的动作摘要，例如读取文件、搜索项目文本、运行质量检查和检查 Git 状态，不展示模型思维链。
- 文件正文、搜索关键词、提交消息和其他自由文本参数不会写入活动摘要；路径会压缩为空白规范的单行文本，并限制长度。
- 审批拒绝使用独立的 `skipped` 状态，不与工具自身的参数或执行失败混淆。

### 终端渲染

- 新增 `TerminalRenderer`，统一协调活动事件、任务计划和模型流式文本。
- 活动状态使用 `[>]`、`[ok]`、`[-]`、`[!]`，兼容 Windows 终端并分别表示执行中、成功、跳过和失败。
- 如果模型先输出文字再调用工具，渲染器会关闭当前回答行；工具结束后的文字会重新显示 `Neil Agent >` 前缀。
- 任务计划更新也会先安全结束当前回答片段，不再依赖散落在 CLI 中的手动换行。
- 增加活动顺序、敏感参数隐藏、审批跳过、流式文本续接和计划插入测试；当前测试总数为 65。
