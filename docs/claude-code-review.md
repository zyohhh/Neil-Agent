# Claude Code 官方文档对照审核（更新于 2026-08-28）
...
5. 自动化测试规模已显著扩大；当前离线门禁 pytest 为 829 项通过（排除 `online` 与 `windows_sandbox_security`），含 Web Workbench 契约与安全回归。
...
26. Web `command_id` 缓存按客户端隔离、浏览器使用不可猜测 ID；非 loopback `LLM_BASE_URL` 须 `LLM_ALLOW_CUSTOM_BASE_URL`；生产 `TrustedHost` 不含 `testserver`（见 [`security-hardening.md`](security-hardening.md) 批次 4）。
...
编号可视化路线（Phase 0A–4）与 Web Workbench P0–P9 均已交付。对照 Claude Code 的应用层缺口按 [`security-hardening.md`](security-hardening.md) 批次推进（批次 1–4 已完成；5–6 为只读 Git 内容过滤、写路径 `O_NOFOLLOW`）。下列为跨路线仍开放的验证与产品边界，不构成新的编号 Phase。
