...
Agent 的实时回答显示在底部 `Output` 区域；成功保存后，中间 Workspace 的“会话历史”显示问题和回答。点击左侧之前的 session 即可查看保存的问答；较长历史点击“继续加载历史”，分页错误可重试。工具日志、thinking 和 Provider 私有状态不进入问答历史；较早消息若已被模型压缩，会明确说明。

正文连续排版，保留段落和列表换行，并根据面板宽度自动折行；活动日志逐条显示。流式分片按正文容量合并，日志不会挤掉回答开头；达到实时显示上限会显示提示，完整已保存回答仍可在历史中查看。Output 的清空按钮只隐藏当前浏览器里已有正文，之后收到的流式内容继续显示；新 turn 重置上一轮实时输出。保持服务运行时刷新页面，可以重新读取服务端仍保留的当前运行输出。

源码更新后需要重新启动 `uv run neil-agent-web` 并刷新页面，以同时加载新的历史 API 与打包前端；只有刷新页面不会更新已运行的 Python 服务。
