...
- 宽终端并列显示树和详情；宽度小于 88 列时隐藏详情。DAG 与回答流默认均衡使用剩余高度，高度小于 36 行时压缩标题和指标并优先扩大回答流；`F2`（或 `Ctrl+O`）可隐藏 DAG、展开回答流，再次触发后无损恢复。resize 会即时切换宽度和高度布局。非交互终端、导入失败或 Textual 启动异常自动回退 `/cockpit` 快照。
