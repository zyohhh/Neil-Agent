...
- Web 每轮仍新建 `Agent`，但会从当前选中的 `SessionStore` 快照恢复历史，并在成功回合后写回；`select_session` / `new_session` 切换会话。启动时自动恢复最近一次已保存会话。`SecurityShield` 与 `ContextTomography` 均通过共享投影 helpers 写入 Web 快照 DTO。详见 [`host-runtime.md`](host-runtime.md)。
