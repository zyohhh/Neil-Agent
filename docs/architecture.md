...
- Textual 使用同一个工作区槽位承载 DAG 和 Context，`F3`（或 `Ctrl+T`）切换，不再增加会压缩回答区的第三个纵向面板。宽屏并列显示五层、裁剪洞察与本地/服务端遥测；窄屏将洞察压缩成三行；矮屏把裁剪、检查点、最大工具结果和历史服务端总量收进边框副标题，完整保留五层和回答区。
...
- Phase 2A 仍未完成；剩余第三批是预算阈值预警和完全本地的 what-if 模拟。
