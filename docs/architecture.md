...
CLI 使用 `TerminalRenderer` 统一处理三类异步输出：Agent 活动事件、任务计划变化和模型流式文字。活动或计划插入时会先结束当前回答行，后续模型文字重新显示回答前缀，避免多个回调直接写终端导致内容粘连。

## 执行活动边界

- 活动事件只描述可观察的高层动作，不展示或推断模型思维链。
- 模型请求会报告开始和请求失败；每个工具会报告执行中、成功、失败或因审批拒绝而跳过。
- 工具成功与失败事件显示本地流程耗时，审批拒绝不伪装成执行失败。
- 已知工具只选择必要的结构化参数生成摘要，例如文件路径、检查类型或暂存文件数量。
- 文件内容、替换文本、搜索关键词、提交消息等自由文本参数不会进入活动事件。
- 未知工具只显示经过单行化和长度限制的工具名称，不回显参数。
