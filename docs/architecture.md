...
- Web 每轮仍新建 `Agent`，但会从当前选中的 `SessionStore` 快照恢复历史，并在成功回合后写回；`select_session` / `new_session` 切换会话。启动时自动恢复最近一次已保存会话。`SecurityShield` 通过 `observe_host_security()` 与 CLI 同源投影到快照 `security` DTO；`ContextTomography` 仍待 richer 投影。详见 [`host-runtime.md`](host-runtime.md)。
