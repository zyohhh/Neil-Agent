...
Neil Agent 是一个运行在终端中的本地 Coding Agent。当前版本能够与 DeepSeek V4 Flash 多轮对话，在瞬时失败后进行可观察的有界重试，加载根目录项目指令，显式压缩长会话，在限定工作区内读写文件、执行检查和创建本地提交，并管理可恢复的本地会话。
...
agent.py
  对话历史、工具循环、活动事件、审批协调、修改后验证工作流
    ...
    ↘ instructions.py
      根 AGENTS.md 的文件、编码、大小和内容边界
    ↘ session.py
      版本化会话快照、原子保存、统计、显式加载与删除
    ↓
llm.py
  DeepSeek Anthropic API、有界重试、流式事件、ToolCall 解析
    ↓
tools/registry.py
  工具注册、参数绑定、预览和执行分发
    ├→ tools/filesystem.py
...
- 启动时只读取 `WORKSPACE_ROOT/AGENTS.md`，当前版本不递归发现子目录指令，也不在运行中自动重载。
- 文件必须是根目录中的真实普通文件，不能是符号链接或解析到其他位置；最大 32768 字节，并在读取前后重复检查路径和实际字节数。
...
- 项目指令不进入 `Message` 历史、会话 JSON 或 `/instructions` 正文输出；`/instructions` 只展示来源、状态和规模。
...
- Agent 从最新历史轮次向前选择连续后缀，同时满足轮数和字符预算。任一轮超出预算时停止，不会拆分工具调用/结果，也不会跳过较新的大轮次去保留更旧的小轮次。
...
- `/compact` 不是自动策略，只有用户明确输入时才调用模型；默认至少需要 3 轮，并完整保留最近 2 轮。
...
- 候选历史由一个固定的用户/助手检查点轮次和最近原始轮次组成，继续满足消息历史与工具调用配对验证，因此沿用会话格式版本 1。
- 候选生成阶段不修改内存；CLI 先原子保存候选，再应用到 Agent。失败、取消、历史变化或保存错误均保留原历史。
...
- `/sessions` 展示有效会话数量、所有 JSON 会话文件的总占用，以及各有效会话的大小；最多仍只列出最近 50 个摘要。
