...
- 候选历史由一个固定的用户/助手检查点轮次和最近原始轮次组成，继续满足消息历史与工具调用配对验证，并使用当前会话格式版本 2 保存。
...
- 会话快照固定存储在 `WORKSPACE_ROOT/.neil-agent/sessions/`，当前写入版本 2；版本 1 仍可严格读取，并在下一次保存或重命名时迁移为版本 2，未知未来版本继续拒绝。
- 版本 2 新增本地标题；首次成功回答后从第一条用户请求确定性生成默认标题，不调用模型。标题可通过 `/rename-session <标题>` 修改，最多 80 个字符并拒绝控制或格式字符。
- 快照字段仅包含会话 ID、标题、创建/更新时间、成功消息历史、任务计划和最近质量检查；不包含 API Key、环境变量、系统提示词或远程同步配置。
...
- `/export [id]` 将严格的版本 1 导出信封写入 `.neil-agent/exports/`；只包含版本 2 会话字段，不包含环境配置、系统提示词和项目指令，且不会覆盖同名文件。
