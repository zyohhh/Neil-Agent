...
| Context | `ContextTomography` + `TokenUsage` | 五层本地估算 + 服务端 usage + Review 断层图 | what-if 模拟、Provider 上限元数据 |
...
P1–P9 已接入真实 Agent、逐工具审批（含 `binding_kind`）、Review、wheel 分发、参考图回归、前端故障恢复、Web 会话连续性、受控模型切换与 guest export 导入；P0 fixture 仍用于视觉与状态回归。`run_readonly_subtask` 由 Web `standard`/`web-safe` 工具面注册，turn worker 传入 `parent_run_id`；**前端无独立子任务 UI**，观测走通用 `runtime_step`。`runtime_step` / 快照 `timeline` 的 `workspace_path` 在投影时折成相对目录（`project_web_runtime_metadata`），不向浏览器发送叶文件名。
