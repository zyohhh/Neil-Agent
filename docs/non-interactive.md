...
approve 在读取并验证 pending 正文**之前**，先根据用户提供的 approval ID 以
独占创建方式写入 `.neil-agent/approvals/consumed/`，随后才校验并移除
pending。因此：

- 同一 ID 只能使用一次。
- pending 损坏、摘要不符、prompt 不符、模型/工具失败、进程崩溃，或外部进程
  临时篡改后恢复正文，都不会恢复授权；必须重新生成预览。
- terminal marker 最多保留 1000 项并保留 24 小时，远长于 pending 的 15 分钟
  有效期；过期 marker 清理后，恢复旧 pending 仍会因自身过期而被拒绝。
